...
public client class Client {
    ...
    // # + pullRequestReviewComment - Pull request review comment create record
    // # + return - Created issue object or Connector error
    // remote function submitPullRequestReview(string repositoryOwner, string repositoryName, int pullNumber , int reviewId, string body, string reviewAction)
    //                        returns error? {
    ...
    //     map<json> submitPullRequestReviewPayload = {
    //         body: body,
    //         event: reviewAction
    //     };
    ...
    //     setHeader(request, self.accessToken);
    //     //Set headers and payload to the request
    //     constructRequest(request, <@untainted> submitPullRequestReviewPayload);

    //     string endpointResource = string `${PATH_SEPARATOR}${GIT_REPOS}${PATH_SEPARATOR}${repositoryOwner}${PATH_SEPARATOR}${repositoryName}${PATH_SEPARATOR}${GIT_PULLS}${PATH_SEPARATOR}${pullNumber}${PATH_SEPARATOR}${GIT_REVIEWS}${PATH_SEPARATOR}${GIT_EVENTS}`;
    //     // Make an HTTP POST request
    //     var response = self.githubRestClient->post(endpointResource, request);
    //     //Check for empty payloads and errors
    //     json validatedResponse = check getValidatedRestResponse(response);
    ...
    # + return - Created issue object or Connector error
    remote function deleteBranch(string repositoryOwner, string repositoryName, string ref)
                           returns error? {
        ...
        string endpointResource = string `${PATH_SEPARATOR}${GIT_REPOS}${PATH_SEPARATOR}${repositoryOwner}${PATH_SEPARATOR}${repositoryName}${PATH_SEPARATOR}${GIT}${PATH_SEPARATOR}${ref}`;
